...
As most branches are usually taken, we initialize the predictor with the biasing bit storage table to $0$ and the the pattern history table to $00$. That way, when the first prediction need to be done, the pattern history table will not agree to the value in the biasing bit storage table, therefore the prediction will be _taken_.
...
Due to a limited time and some benchmarks taking too much time to finish, each one was executed with small input sets and limited to 60 minutes of runtime, after which all the statistic files were collected and ROI statistics were analyzed and plotted.
...
The first test consists of increasing the size of the pattern and bias tables, together with a length of the branch hisory register equal to the exponent of the size of the tables, that is,

$$tableSize = 2 ^ {bhrSize}$$

The second test is to increase the size of the pattern and bias tables, but to check the influence of the branch direction, we have reduced the size of the branch hisory register by two bits, so that the 2 MSB of the index to the pattern table belong to the branch direction, that is,

$$tableSize = 2 ^ {bhrSize + 2}$$
...
In Figure 4 we can see that the accuracy increases to a greater number of bits, but up to some extent. $3$ bits are better than $2$ bits, but $4$ bits fall far below the $2$ bit counter. This is because the saturation counters are not used to make the prediction itself, but are used to check if the branch pattern is in accordance with the bias. Therefore, by increasing the number of bits the counter can become more accurate, however, there is a break-even point where the counter takes so long to agree with the bias bit that it leads to a worse accuracy in the prediction.
...
1. The larger the tables, the greater the accuracy.
2. Increasing the number of biasing bits leads to a better accuracy.

### Table sizes

The first test was done in a similar fashion to the one performed in the Agree predictor. It consists of increasing the size of the , together with a length of the branch hisory register equal to the exponent of the size of the tables, that is,

$$tableSize = 2 ^ {bhrSize}$$

![Perceptron predictor accuracy](images/perceptron_accuracy.png)

In Figure 5 we can see that the accuracy follows a logarithmic trend, which seems to converge to an asymptote around 93% accuracy. We will later analyse this result in comparison with other predictors and present possible solutions and improvements.

Again, similar to the previous predictor, if the number of entries and the length of the history register increases, the accuracy increases, as would be expected and confirming our hypothesis. This increase in accuracy is due to the fact that the perceptrons are able to analyze the behavior of the branches through their training, since the bits that have a higher correlation with the branch result will have higher values than the bits with lower correlation. When more bits are available, more correlations can be found. Also, when more perceptrons are in use, less interference occurs, also increasing the accuracy.
...
Therefor, the greater the number of entries and the longer the branch history register, the less interference and the greater the accuracy, but the greater the length is accompanied by a loss of performance if the training is not optimized.
...
This can be attributed to the fact that, even though we've provided the perceptron with some bias or "help", the perceptron still finds useful correlations when that first $k$ bits are biased and set to true, an that this bias bits have little influence on the final prediction.
...
Another observation we made is that when the branch predictors are more complex, the time they take to make a prediction increases, as more cycles are required to perform lookups and/or training. Improvements on our implementations will be discussed in the conclusions.
...
Finally, we could also make use of longer branch history register lengths, to make better use of the characteristics of a perceptron but again, taking the resource budget into account and implementing it in an efficient way.